...
export class TimeThisWeekComponent implements OnInit {
  ...
  private updateTimes(activity: TimeActivity[]) {
    ...
    this.times = this.times.map((val, i) => {
      ...
      return activityPeace ? TimeThisWeekComponent.convertTime(activityPeace.time) : '0';
    });

    this.totalWeekTimeText = TimeThisWeekComponent.convertTime(this.totalWeekTime);
  }
